...
static int xcallback(lua_State* L) {
  ...
  lua_insert(L, 1);
  if (up_values > 2) { /* if the function has bound parameters */
    lua_rotate(L, 2, up_values - 2); /* rotate the calling parameters after the binding parameters */
  }
  int n = lua_gettop(L) - 1; /* calculate the number of all parameters */
  int status = luaC_pcall(L, n, LUA_MULTRET);
  if (status != LUA_OK) {
...
static int luaf_xbind(lua_State* L) {
...
LUAC_API int luaC_open_bind(lua_State* L) {
  const luaL_Reg methods[] = {
    ...
    { "xbind",  luaf_xbind  }, /* xbind(name, self [, arg1, ...]) */
    { NULL,     NULL        }
  };
